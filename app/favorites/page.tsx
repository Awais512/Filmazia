...
import { MovieGridSkeleton, EmptyState, Modal, Button, Input } from '@/components/ui';
import { Movie, TVShow } from '@/lib/tmdb-types';
import { tmdb } from '@/lib/tmdb-api';
import { Folder, FolderPlus, Trash2 } from 'lucide-react';

function useHydratedFavorites() {
...
export default function FavoritesPage() {
  const { hydrated, store } = useHydratedFavorites();
  ...
  // Fetch items only once when hydrated and items exist
  useEffect(() => {
    if (!hydrated) return;
  ...
  }, [hydrated, favoriteItems.length]); // Only depend on hydration and item count
